...
protocol HandLandmarkerServiceLiveStreamDelegate: AnyObject {
  func handLandmarkerService(_ handLandmarkerService: HandLandmarkerService,
                             didFinishDetection result: ResultHandBundle?,
                             error: Error?)
}
...
class HandLandmarkerService: NSObject {
   ...
   **/
  func detect(image: UIImage) -> ResultHandBundle? {
    ...
    do {
      ...
      return ResultHandBundle(inferenceTime: inferenceTime, handLandmarkerResults: [result])
    } catch {
        print(error)
        return nil
    }
  ...
  func detect(
    ...
    inferenceIntervalInMilliseconds: Double) async -> ResultHandBundle? {
    let startDate = Date()
...
extension HandLandmarkerService: HandLandmarkerLiveStreamDelegate {
  func handLandmarker(
    ...
    error: Error?) {
      let resultBundle = ResultHandBundle(
