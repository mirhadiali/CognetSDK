...
protocol FaceLandmarkerServiceLiveStreamDelegate: AnyObject {
  func faceLandmarkerService(_ faceLandmarkerService: FaceLandmarkerService,
                             didFinishDetection result: ResultFaceBundle?,
                             error: Error?)
}
...
class FaceLandmarkerService: NSObject {
  ...
  static func liveStreamFaceLandmarkerService(
    ...
    delegate: FaceLandmarkerDelegate) -> FaceLandmarkerService? {
        ...
        print(modelPath)
        print(numFaces)
        print(minFaceDetectionConfidence)
        print(minFacePresenceConfidence)
        print(minTrackingConfidence)
        print(liveStreamDelegate)
        print(delegate)
        print("End Face values ====")
   ...
   **/
  func detect(image: UIImage) -> ResultFaceBundle? {
    ...
    do {
      ...
      return ResultFaceBundle(inferenceTime: inferenceTime, faceLandmarkerResults: [result])
    } catch {
        print(error)
        return nil
    }
  ...
  func detect(
    ...
    inferenceIntervalInMilliseconds: Double) async -> ResultFaceBundle? {
    let startDate = Date()
...
extension FaceLandmarkerService: FaceLandmarkerLiveStreamDelegate {
  func faceLandmarker(
    ...
    error: Error?) {
      let resultBundle = ResultFaceBundle(
