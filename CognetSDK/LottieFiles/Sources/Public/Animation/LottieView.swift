...
struct LottieView<Placeholder: View>: UIViewConfiguringSwiftUIView {

  // MARK: Lifecycle

  /// Creates a `LottieView` that displays the given animation
  init(animation: LottieAnimation?) where Placeholder == EmptyView {
  ...
  /// }
  /// ```
  init(dotLottieFile: DotLottieFile?) where Placeholder == EmptyView {
  ...
  init(_ loadAnimation: @escaping () async throws -> LottieAnimation?) where Placeholder == EmptyView {
  ...
  /// }
  /// ```
  init(_ loadDotLottieFile: @escaping () async throws -> DotLottieFile?) where Placeholder == EmptyView {
  ...
  /// If you wish to call `loadAnimation` again at a different time, you can use `.reloadAnimationTrigger(...)`.
  /// While the animation is loading, the `placeholder` view is shown in place of the `LottieAnimationView`.
  init(_ loadAnimation: @escaping () async throws -> LottieAnimationSource?) where Placeholder == EmptyView {
    self.init(loadAnimation, placeholder: EmptyView.init)
  }
  ...
  func configure<Property: Equatable>(
    _ property: ReferenceWritableKeyPath<LottieAnimationView, Property>,
    to value: Property)
    -> Self
  {
  ...
  func configure(_ configure: @escaping (LottieAnimationView) -> Void) -> Self {
    var copy = self
  ...
  func play(loopMode: LottieLoopMode = .playOnce) -> Self {
  ...
  func play(_ playbackMode: LottiePlaybackMode) -> Self {
    self.playbackMode(playbackMode)
  }

  /// Returns a copy of this view playing with the given playback mode
  func playing(_ mode: LottiePlaybackMode.PlaybackMode) -> Self {
  ...
  func playing(loopMode: LottieLoopMode) -> Self {
    playbackMode(.playing(.fromProgress(nil, toProgress: 1, loopMode: loopMode)))
  }
  ...
  func paused(at state: LottiePlaybackMode.PausedState = .currentFrame) -> Self {
    playbackMode(.paused(at: state))
  }

  /// Returns a copy of this view using the given `LottiePlaybackMode`
  func playbackMode(_ playbackMode: LottiePlaybackMode) -> Self {
    var copy = self
  ...
  func animationSpeed(_ animationSpeed: Double) -> Self {
    var copy = self
  ...
  func animationDidLoad(_ animationDidLoad: @escaping (LottieAnimationSource) -> Void) -> Self {
    var copy = self
  ...
  func animationDidFinish(_ animationCompletionHandler: LottieCompletionBlock?) -> Self {
    var copy = self
  ...
  func backgroundBehavior(_ value: LottieBackgroundBehavior) -> Self {
    configure { view in
  ...
  func configuration(_ configuration: LottieConfiguration) -> Self {
    var copy = self
  ...
  func logger(_ logger: LottieLogger) -> Self {
    var copy = self
    copy.logger = logger
    return copy
  }
  ...
  func imageProvider<ImageProvider: AnimationImageProvider & Equatable>(_ imageProvider: ImageProvider) -> Self {
    var copy = self
  ...
  /// The image provider must be `Equatable` to avoid unnecessary state updates / re-renders.
  func textProvider<TextProvider: AnimationKeypathTextProvider & Equatable>(_ textProvider: TextProvider) -> Self {
    var copy = self
  ...
  /// Returns a copy of this view with its image provider updated to the given value.
  /// The image provider must be `Equatable` to avoid unnecessary state updates / re-renders.
  func fontProvider<FontProvider: AnimationFontProvider & Equatable>(_ fontProvider: FontProvider) -> Self {
    var copy = self
  ...
  func valueProvider<ValueProvider: AnyValueProvider & Equatable>(
  ...
  func currentProgress(_ currentProgress: AnimationProgressTime?) -> Self {
  ...
  ///    will continue playing uninterrupted.
  func currentFrame(_ currentFrame: AnimationFrameTime?) -> Self {
  ...
  func reloadAnimationTrigger(_ value: some Equatable, showPlaceholder: Bool = true) -> Self {
    var copy = self
  ...
  func getRealtimeAnimationProgress(_ realtimeAnimationProgress: Binding<AnimationProgressTime>?) -> some View {
  ...
  ///  - If the binding is `nil`, the `TimelineView` will be paused and no updates will occur to the binding.
  @available(iOS 15.0, tvOS 15.0, macOS 12.0, *)
  func getRealtimeAnimationFrame(_ realtimeAnimationFrame: Binding<AnimationFrameTime>?) -> some View {
  ...
  func dotLottieConfigurationComponents(
