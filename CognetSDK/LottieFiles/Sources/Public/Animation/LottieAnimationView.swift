...
enum LottieBackgroundBehavior {
  ...
  static func `default`(for renderingEngine: RenderingEngine) -> LottieBackgroundBehavior {
...
extension LottieLoopMode: Equatable {
  static func ==(lhs: LottieLoopMode, rhs: LottieLoopMode) -> Bool {
    switch (lhs, rhs) {
...
@IBDesignable
class LottieAnimationView: LottieAnimationViewBase {
  ...
  func play(_ mode: LottiePlaybackMode.PlaybackMode, completion: LottieCompletionBlock? = nil) {
  ...
  var animateLayoutChangesWithCurrentCoreAnimationContext = true
  ...
  var valueProviders: [AnimationKeypath: AnyValueProvider] {
  ...
  /// }
  /// ```
  var animationLoaded: ((_ animationView: LottieAnimationView, _ animation: LottieAnimation) -> Void)? {
    didSet {
      if let animation {
  ...
  var maskAnimationToBounds: Bool {
  ...
  var currentPlaybackMode: LottiePlaybackMode? {
  ...
  var mainThreadRenderingEngineShouldForceDisplayUpdateOnEachFrame: Bool {
  ...
  /// ```
  func addSubview(_ subview: AnimationSubview, forLayerAt keypath: AnimationKeypath) {
