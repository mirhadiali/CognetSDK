...
class LottieAnimationLayer: CALayer {
  ...
  func pause(at state: LottiePlaybackMode.PausedState) {
    switch state {
    case .currentFrame:
      removeCurrentAnimation()
  ...
  /// - Parameter completion: A closure that is called after
  ///   an animation triggered by this method completes.
  func play(_ playbackMode: LottiePlaybackMode.PlaybackMode, completion: LottieCompletionBlock? = nil) {
    switch playbackMode {
    case .fromProgress(let from, let to, let loopMode):
      play(
        fromProgress: from,
  ...
  private(set) var currentPlaybackMode: LottiePlaybackMode?

  /// Value Providers that have been registered using `setValueProvider(_:keypath:)`
  private(set) var valueProviders = [AnimationKeypath: AnyValueProvider]()
  ...
  var animationLayerDidLoad: ((_ animationLayer: LottieAnimationLayer, _ renderingEngine: RenderingEngineOption) -> Void)?
  ...
  var animationLayer: CALayer? { rootAnimationLayer }
  ...
  /// }
  /// ```
  var animationLoaded: ((_ animationLayer: LottieAnimationLayer, _ animation: LottieAnimation) -> Void)? {
    didSet {
      if let animation {
        animationLoaded?(self, animation)
      }
  ...
  var maskAnimationToBounds = true {
    didSet {
  ...
  ///
  /// Defaults to `false`
  var shouldRasterizeWhenIdle = false {
    didSet {
  ...
  var realtimeAnimationFrame: AnimationFrameTime {
    isAnimationPlaying ? rootAnimationLayer?.presentation()?.currentFrame ?? currentFrame : currentFrame
  }

  /// Returns the current animation frame while an animation is playing.
  var realtimeAnimationProgress: AnimationProgressTime {
    if let animation {
  ...
  var currentRenderingEngine: RenderingEngine? {
  ...
  var mainThreadRenderingEngineShouldForceDisplayUpdateOnEachFrame = false {
    didSet {
  ...
  func convert(_ rect: CGRect, toLayerAt keypath: AnimationKeypath?) -> CGRect? {
  ...
  /// - Parameter toLayerAt: The keypath used to find the layer.
  func convert(_ point: CGPoint, toLayerAt keypath: AnimationKeypath?) -> CGPoint? {
    guard let animationLayer = rootAnimationLayer else { return nil }
    guard let keypath else {
  ...
  func setNodeIsEnabled(isEnabled: Bool, keypath: AnimationKeypath) {
    guard let animationLayer = rootAnimationLayer else { return }
  ...
  func updateAnimationForBackgroundState() {
  ...
  func updateAnimationForForegroundState(wasWaitingToPlayAnimation: Bool) {
    if let currentContext = animationContext {
