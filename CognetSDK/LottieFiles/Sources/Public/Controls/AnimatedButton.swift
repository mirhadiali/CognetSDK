...
class AnimatedButton: AnimatedControl {
  ...
  func setPlayRange(fromProgress: AnimationProgressTime, toProgress: AnimationProgressTime, event: LottieControlEvent) {
    rangesForEvents[event.id] = (from: fromProgress, to: toProgress)
  }

  /// Sets the play range for the given UIControlEvent.
  func setPlayRange(fromMarker fromName: String, toMarker toName: String, event: LottieControlEvent) {
    if
